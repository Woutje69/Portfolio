...
if (isset($_POST['btnSubmit'])) {
    ...
    if ($allOk) {
        $stmt = $db->exec('INSERT INTO messages (sender, message, added_on) VALUES (\'' . $mail . '\',\'' . $message . '\',\'' . (new DateTime())->format('Y-m-d H:i:s') . '\')');

        // the query succeeded, redirect to this very same page
        if ($db->lastInsertId() !== 0) {
            header('Location: formchecking_thanks.php?name=' . urlencode($mail));
            exit();
        } // the query failed
        else {
            echo 'Databankfout.';
...
<main class="container">
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
        ...
        <div>
            ...
            <input type="mail" id="mail" name="mail" value="<?php echo $mail; ?>" class="input-text"/>
